...
**{Sync}:** POST /sync-complete with {"text": "...", "session": "S{N}"}. Compact mode (default): CORE.md returns as _core_summary (sections + counts, body stripped — full body via GET /read), ACTIVE.md summarized if >15KB, vine resonance capped at 5 matches per seeder, handoff warns if >10KB. One call returns: active entity + config + mandatory entity files (class-dependent; override via entity.json "mandatory" array) + deferred file manifest (name + size for remaining files) + linked identities + armed_seeders + vine resonance + vine_processing + tracker updates + obligations. Deferred files available via GET /read on demand Ã¢â‚¬â€ pull when conversation topic requires them. Service all obligations in the response.
...
**File operations:** All writes to files inside BOND_ROOT go through daemon /file-op via daemon_fetch. Never use filesystem MCP tools to write BOND files directly. Shape: POST /file-op with {"op": "replace|append|write|delete", "path": "relative/to/bond_root", ...params, "initiator": "claude"}. Replace: needs "old" + "new". Append: needs "content" (optional "position"/"marker"). Write: needs "content" (D15 gate applies â€” may return hold requiring "confirmed": true). Delete: needs "confirmed": true (mandatory gate, shadow .bak, protected paths rejected). Paths are relative to BOND_ROOT (e.g. "doctrine/BOND_OVERHAUL/ACTIVE.md").
